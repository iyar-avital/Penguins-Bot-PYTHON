...
def get_closest_my_iceberg(game, my_iceberg):
    my_icebergs = [ice for ice in game.get_my_icebergs() if ice != my_iceberg]
    return min(my_icebergs, key = lambda ice: ice.get_turns_till_arrival(my_iceberg))
...
def get_destination_pinguin_amount(game, type, my_iceberg, destination, icebergs_attacked, bridges, natural_icebergs):
    ...
    # enemy iceberg case
    if type == 'winner':
        return my_iceberg.penguin_amount - 5
...
def is_win(game, my_iceberg):
    ...
    if len(all_enemy_icebergs) == 1:
        ...
        penguins = [penguins_group.penguin_amount for penguins_group in game.get_enemy_penguin_groups() if penguins_group.decoy == False and penguins_group.destination == enemy_iceberg]
...
def is_loss(game, enemy_iceberg, action_cost):
    all_my_icebergs = game.get_my_icebergs()
    all_enemy_icebergs = game.get_enemy_icebergs()
    if len(all_my_icebergs) == 1:
        penguins = [penguins_group.penguin_amount for penguins_group in game.get_my_penguin_groups() if penguins_group.decoy == False]
        my_iceberg = all_my_icebergs[0]
        distance = my_iceberg.get_turns_till_arrival(enemy_iceberg)
        penguin_amount_need = my_iceberg.penguin_amount + distance*my_iceberg.penguins_per_turn - action_cost
        penguin_amount_need += sum(penguins)
        if enemy_iceberg.penguin_amount > penguin_amount_need:
            return True
    return False
    
def danger_to_act(game, my_iceberg, action_cost):
    if len(game.get_enemy_icebergs()) == 1:
        if is_loss(game, game.get_enemy_icebergs()[0], action_cost):
            return True
    return False
...
def wait(game, my_iceberg):   
    if len(game.get_enemy_icebergs()) == 1 and len(game.get_my_icebergs()) == 1:
        if game.get_enemy_icebergs()[0].penguin_amount == game.get_my_icebergs()[0].penguin_amount:
            if game.get_enemy_icebergs()[0].penguins_per_turn == game.get_my_icebergs()[0].penguins_per_turn:
                return True
...
def do_turn(game):
    ...
    for my_iceberg in game.get_my_icebergs():
        ...
        if is_winner and my_iceberg == winner:
            if winner.can_create_bridge(losser):
                winner.create_bridge(losser)
                return
        ...
        if my_iceberg.penguin_amount == my_iceberg.max_penguins and not is_make_me_winner and not is_enemy:
            if natural_icebergs:
                destination = get_closest_natural_iceberg(game, game.get_enemy_icebergs()[0], natural_icebergs)
                destination_penguin_amount = 1
                print "max send natural"
                my_iceberg.send_penguins(destination, destination_penguin_amount)
 
            else:
                destination = get_closest_my_iceberg(game, my_iceberg)
                destination_penguin_amount = 1
                print "max send my"
                my_iceberg.send_penguins(destination, destination_penguin_amount)
    ...
    for i in send:
        ...
        num = i[2]
        type = i[-1]
        print my_ice, des, num
        if danger_to_act(game, my_ice, num) == False or type == "winner":
            my_ice.send_penguins(des, num)
    
    for my_ice in upgrades:
        if my_ice not in send and danger_to_act(game, my_ice, my_ice.upgrade_cost) == True:
            if DEBUG:
                print my_iceberg, "upgrade from ", my_iceberg.level, " to", (my_iceberg.level + 1)
