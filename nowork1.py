...
def turn(game):
    ...
    print game.get_enemy().__str__()
    if "Us" in game.get_enemy().__str__() and  "The 3 Of" not in game.get_enemy().__str__():
        ambush=None
        decoy=None
        ...
        for my_iceberg in game.get_my_icebergs():
            ...
            # Icebergs Lists
            neturalSorted = sorted(game.get_neutral_icebergs(), key=lambda x: x.penguin_amount*0.02 + my_iceberg.get_turns_till_arrival(x)*0.98)
            enemySorted = sorted(game.get_enemy_icebergs(), key=lambda x:  x.penguin_amount*0.1 + my_iceberg.get_turns_till_arrival(x)*0.9)
            ...
            bridgeDestination = bridgeDest(game, my_iceberg, game.get_my_penguin_groups())  
            protectSorted = sorted(iceInDanger(game, my_iceberg), key=lambda x: x.penguin_amount*0.2+ my_iceberg.get_turns_till_arrival(x)*0.8)
            neutralAndEnemySorted=sorted(game.get_enemy_icebergs()+game.get_neutral_icebergs(), key=lambda x:  x.penguin_amount*0.1 + my_iceberg.get_turns_till_arrival(x)*0.9)
            if not decoy:
                decoy=neutralAndEnemySorted[0]
            # Divide into small groups, if the enemy behaves that way
            y = 1
            ...
            elif staticState(game):
                y += 1
                
            neutral = neturalSorted[0] if neturalSorted else None
            enemy = enemySorted[0] if enemySorted else None
            protect = protectSorted[0] if protectSorted else None  
            bridge = bridgeDestination[0] if bridgeDestination else None
            ...
            # Find destination
            if protect and my_iceberg.get_turns_till_arrival(protect)<=10 and my_iceberg.get_turns_till_arrival(protect)<=my_iceberg.get_turns_till_arrival(enemy)  :  
                print(str(my_iceberg.id) +"protect")
                destination = protect
            
            elif not ambush and attackEnemy(game, my_iceberg) and not have_more_icebergs(game) and my_iceberg.get_turns_till_arrival(attackEnemy(game, my_iceberg))<=10: 
                print(str(my_iceberg.id) +"attack")
                destination = attackEnemy(game, my_iceberg)
                   
            elif game.get_neutral_icebergs() and not checkDestination(game.get_enemy_penguin_groups(), mySorted) and my_iceberg.get_turns_till_arrival(neutral) <= 8:
                if len(game.get_neutral_icebergs()) > 1 and groups(neturalSorted[0], game.get_my_penguin_groups()):
                    destination = neturalSorted[1]
                else: 
                    destination = neturalSorted[0]       
            
            elif game.get_neutral_icebergs()  and my_iceberg.get_turns_till_arrival(neturalSorted[0])<=10:
                print(str(my_iceberg.id) +"neutral")
                destination = neturalSorted[0]  
                ...
                if not ambush:
                    ambush = enemy
                print(str(my_iceberg.id) +"ambush")
                destination = ambush if ambush  else enemy #and (game.turn>60) else enemy
            else:
                destination=neutralAndEnemySorted[0]
            if my_iceberg.get_turns_till_arrival(destination)>=25:
                destination=neutralAndEnemySorted[0]
            d_owner, d_amount = IceState(game, destination, my_iceberg.get_turns_till_arrival(destination))
            destination_penguin_amount = d_amount//y if d_amount > my_penguin_amount  and d_owner == game.get_enemy()  else d_amount
            destination_penguin_amount += bonus
            # Act
            m_owner, m_amount = IceState(game, my_iceberg, my_iceberg.get_turns_till_arrival(destination), destination_penguin_amount)
            if enemyTerritory(game, my_iceberg, destination_penguin_amount) or (groups(my_iceberg, game.get_enemy_penguin_groups()) and m_owner == game.get_enemy()): 
                print(str(my_iceberg.id) +"nothing")
                continue
            elif bridge and my_iceberg.can_create_bridge(bridge) and ((my_iceberg.level > 1 and my_iceberg.get_turns_till_arrival(bridge) >= 5 and groups(bridge,game.get_my_penguin_groups())>=6)):# or IceState(game, bridge, my_iceberg.get_turns_till_arrival(bridge))[0] == game.get_enemy()): 
                my_iceberg.create_bridge(bridge)
                continue
            
            elif (have_more_icebergs(game)) and canUpgrade(game, my_iceberg) and not (game.turn < 40 and my_iceberg.level >= 2) :
                my_iceberg.upgrade()
                continue
            elif (((have_more_icebergs(game) ) and groups(game.get_bonus_iceberg(),game.get_enemy_penguin_groups())) or (game.get_bonus_iceberg().owner == game.get_enemy())) and my_iceberg in bonusSorted[0: len(bonusSorted)//2] :
                d_owner, d_amount = IceState(game, game.get_bonus_iceberg(),my_iceberg.get_turns_till_arrival(game.get_bonus_iceberg()))
                if d_owner != game.get_myself and my_iceberg.penguin_amount > d_amount :
                    my_iceberg.send_penguins(game.get_bonus_iceberg(), d_amount+1)
            
           
            
            elif enemyDecoyPenguins(game) and  my_iceberg.penguin_amount +2 > destination_penguin_amount*2   and my_iceberg.can_send_decoy_penguins(destination, neutralAndEnemySorted[len(neutralAndEnemySorted)//2], destination_penguin_amount) :
                ...
                my_iceberg.send_decoy_penguins(destination,neutralAndEnemySorted[len(neutralAndEnemySorted)//2], destination_penguin_amount +1) 
    ...
    if "The 3 Of Us" in game.get_enemy().__str__():
        for my_iceberg in game.get_my_icebergs():
            min_ices = sorted(game.get_neutral_icebergs(), key = lambda ice : ice.level*ice.penguin_amount*ice.get_turns_till_arrival(my_iceberg))
            if len(min_ices) == 1:
                min_ice = min_ices[0]
        ...
        for i in game.get_all_icebergs():
            print(i,i.penguin_amount, j , j.penguin_amount, distance_between_iceberges(game, i, j) , i.get_turns_till_arrival(j))
        ...
        is_help=[]
        is_protecting=[]
        my_iceberg_for_bridge=filter(lambda x: iceberg_future(game, x)[2]>=0 and safe_to_send(game, x, game.iceberg_bridge_cost, x), game.get_my_icebergs())
        #filter all iceberg that need help or need to upgrade
        icebergs=filter(lambda x: iceberg_future(game, x)[2]<=0 or (x.owner==game.get_myself() and x.level<4), game.get_all_icebergs())
        for my_iceberg in my_iceberg_for_bridge:
            ...
            if  int(bridge_value(game, my_iceberg, destinations[0]))>=0:
                ...
                continue
        if iceberg_future(game,game.get_bonus_iceberg())[2]<=0:# if bonus iceberg is not going to be mine.
        ...
        for iceberg in sorted_all_icebergs:
            ...
            for my_iceberg in my_icebergs:
                ...
                closest_iceberg= my_icebergs[0]
                if my_iceberg in is_helping:#  or my_iceberg in is_helping:# if my_iceberg has already done some action- skip
                    continue
                elif iceberg==sorted_all_icebergs[0]:
                    is_help.append(closest_iceberg)
                destination_penguin_amount=penguins_required_real(game, iceberg, my_iceberg)#p_bridge(game, my_iceberg, iceberg)#penguins_required_real(game, iceberg, my_iceberg)
                if my_iceberg.penguin_amount>destination_penguin_amount:
                    if safe_to_send(game, my_iceberg, destination_penguin_amount + 1, iceberg) and (my_iceberg.get_turns_till_arrival(iceberg))/2+1<=300-game.turn:# or iceberg_value(game,my_iceberg)>iceberg_value(game, iceberg):
                        print my_iceberg, "sends", (destination_penguin_amount + 1), "penguins to", iceberg
                        my_iceberg.send_penguins(iceberg, destination_penguin_amount + 1)
                        if iceberg==sorted_all_icebergs[0]:
                            is_help.remove(closest_iceberg)
                        is_helping.append(my_iceberg)
                        break
        return
