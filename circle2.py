...
def circle(game):
    ...
    :type game: Game
    """
    for i in game.get_enemy_icebergs():
        for j in game.get_my_icebergs():
            print "enemy", i, "my", j, "distance", i.get_turns_till_arrival(j)
    ...
    if game.turn == 1:
        for ice in game.get_my_icebergs():
            enemy = get_closest_enemy_iceberg(game, ice)
            our_des = enemy
            if ice.get_turns_till_arrival(enemy) == 12:
                ice.create_bridge(enemy)
            else:
                ice.send_penguins(enemy, ice.penguin_amount-1)
        return
    
    if game.turn == 2:
        for ice in game.get_my_icebergs():
            if ice.get_turns_till_arrival(our_des) == 12:
                ice.send_penguins(our_des, ice.penguin_amount-1)
        return

    if game.turn == 7:
        help_enemy_attack = [pg for pg in game.get_enemy_penguin_groups() if pg.destination == our_des]
        if help_enemy_attack != [] and help_enemy_attack[0].source.bridges != []:
            for ice in game.get_my_icebergs():
                if ice.get_turns_till_arrival(our_des) == 8:
                    ice.create_bridge(our_des)
        return
    ...
    for my_iceberg in game.get_my_icebergs():
        ...
        """"all_enemy = game.get_enemy_icebergs()
        for pg in game.get_enemy_penguin_groups():
            if who_ice_going_to_be(game, pg.destination)[0] == 'enemy' and pg.destination not in all_enemy:
                all_enemy += [pg.destination]
        
              
        
        if len(all_enemy) >= len(game.get_my_icebergs()) and (is_max_level or is_enemy):
