...
def penguins_group_didnt_do_just_one(game, pg, doing_just1):
    if pg in doing_just1.keys():
        return 0
    all_pg = [p for p in game.get_enemy_penguin_groups() if p.destination == pg.destination and p not in doing_just1.keys()]
    return len(all_pg)
    
def just_one(game, my_iceberg, doing_just1):
    for pg in game.get_enemy_penguin_groups():
        print pg, "pg"
        print doing_just1, "doing_just1"
        pg_to_same_dest = penguins_group_didnt_do_just_one(game, pg, doing_just1) 
        print "pg_to_same_dest", pg_to_same_dest
        if pg.destination in game.get_neutral_icebergs() and pg_to_same_dest == 1 and not pg.decoy:
            ...
            if pg.penguin_amount - pg.destination.penguin_amount == 1:
                ...
                my_to_it -= sum([doing_just1[p] for p in doing_just1.keys() if p.destination == pg.destination])
...
def danger_to_upgrade(game, my_iceberg, upgrade_cost, new_level):
    ...
    if len(all_enemy_icebergs) == 1:
        for my_ice in game.get_my_icebergs():
            print "my_ice.upgrade_cost ", my_ice.upgrade_cost 
            print "my_ice.upgrade_value", my_ice.upgrade_value, "my_ice.penguins_per_turn", my_ice.penguins_per_turn
            if my_ice.penguin_amount - my_ice.upgrade_cost + (my_ice.upgrade_value+ my_ice.penguins_per_turn)*my_ice.get_turns_till_arrival(all_enemy_icebergs[0]) < all_enemy_icebergs[0].penguin_amount:
                return True
...
def finish_time(game, my_iceberg):
    ...
    my_penguin_groups_to_enemy = [pg for pg in game.get_my_penguin_groups() if pg.destination in game.get_enemy_icebergs()]
    if my_penguin_groups_to_enemy !=[]:
        """sum_enemy = 0
        sum_my = 0
        need_bridge = dict()
        bridge_to = None
        for ice in game.get_all_icebergs():
            result = who_ice_going_to_be(game, ice)
            if result[0] == 'my' and result[2] <= game.max_turns-game.turn:
                sum_my += 1
            if result[0] == 'enemy':
                sum_enemy += 1
            elif result[0] == 'my' and result[2] > game.max_turns-game.turn:
                need_bridge[ice]= result[2]
    
        print "need_bridge", need_bridge
        print "sum_enemy", sum_enemy, "sum_my", sum_my
        if sum_enemy > sum_my and need_bridge != []:
            for pg in game.get_my_penguin_groups():
                if pg.destination in need_bridge.keys():
                    if need_bridge[pg.destination] // 2 <= game.max_turns-game.turn:
                        bridge_to = pg.destination
                        continue"""
        bridge_to = None

        for pg in my_penguin_groups_to_enemy:
            if pg.turns_till_arrival > game.max_turns-game.turn and pg.turns_till_arrival//2 <= game.max_turns-game.turn:
                bridge_to = pg.destination
                
        if bridge_to and my_iceberg.can_create_bridge(bridge_to):
            if [my_iceberg,bridge_to] not in [br.get_edges() for br in [br for br in my_iceberg.bridges]]:
                my_iceberg.create_bridge(bridge_to)
                return True
    ...
    distance1 = 0
    if nat1:
        distance1 = ice.get_turns_till_arrival(nat1) // 2
        print nat1, nat1.penguin_amount, distance1
    
    ####### ICE1 ########
    if game.turn == game.max_turns-distance1-1:
        ice_end = ice
        nat1_end = nat1
        distance1_end = distance1 
        if ice_end.can_create_bridge(nat1_end):
            ice_end.create_bridge(nat1_end)
        else:
            distance1_end = None
        return True
    
    elif distance1_end and game.turn == game.max_turns-distance1_end:
        print "need to send"
        if ice_end.can_send_penguins(nat1_end, int((nat1_end.penguin_amount)*1.5)):
            ice_end.send_penguins(nat1_end, int((nat1_end.penguin_amount)*1.5))
            return True
    
    elif distance1_end and game.turn == game.max_turns-distance1_end-1+ice_end.max_bridge_duration-1:
        ice_end.create_bridge(nat1_end)
        return True
...
def tricky(game):
    ...
    for my_iceberg in game.get_my_icebergs():
        ...
        ene = game.get_enemy_icebergs()[0]

        print my_iceberg, my_iceberg.penguin_amount, "my_iceberg"
        ...
        if len(game.get_enemy_icebergs()) == 1 and len(game.get_my_icebergs()) == 1 and not is_make_me_winner:
            if len(game.get_enemy_penguin_groups()) == 0:
                if my_iceberg.can_upgrade() and not danger_to_upgrade(game, my_iceberg, my_iceberg.upgrade_cost, my_iceberg.level+1):
                    my_iceberg.upgrade()
                    return
                else:
                    return
    ...
    for my_ice in game.get_my_icebergs():
        ...
        just_1 = just_one(game, my_ice, just_one_send)
        if just_1: 
            print "yhe, tyr just 1"
            dest = just_1[0].destination
            dest_penguin_amount = just_1[1] + 1
            if [p for p in just_one_send.keys() if p.destination == dest]:
                dest_penguin_amount = just_1[0].penguin_amount
            
            all_he_sent = sum([d[2] for d in send if d[0] == my_ice])
            if my_ice.penguin_amount - 3 > all_he_sent:
                print dest, "just one to natural"
                if my_ice.penguin_amount < dest_penguin_amount and my_ice.penguin_amount >= dest_penguin_amount- my_ice.penguins_per_turn*2:
                    dest_penguin_amount = my_ice.penguin_amount - 3
                print "send", dest_penguin_amount+2
                just_one_send[just_1[0]] = dest_penguin_amount + 2
                my_ice.send_penguins(dest, dest_penguin_amount+2)
